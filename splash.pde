...
int NUMBER_COLORS = (int) random(3, 5);
...
void setup() {
    ...
    ellipseMode(CENTER);
    pixelDensity(displayDensity()); // Set the pixel density to the screen's native density
    blendMode(BLEND); // Set the blend mode to BLEND
    colorMode(HSB, 360, 100, 100, 100);
    background(0, 0, 100);
    noLoop();
    // Replace with the path to your local font file, or use createFont to use
    // default fonts
    font = createFont("resources/abeezee.ttf", 14);
...
void circles(color[] clr, int seed) {
    ...
    for (float x = 0; x <= width; x += spacing) {
        for (float y = 0; y <= height; y += spacing) {
            float radius = noise(x * NOISE_SCALE + 100 * seed, y * NOISE_SCALE + 100 * seed) * CIRCLE_SCALE - 4; // Drive
                                                                                                                 // radius
                                                                                                                 // by
                                                                                                                 // noise,
                                                                                                                 // adjust
...
void addDesc(color[][] palette) {
    ...
    fill(palette[0][0], palette[0][1], palette[0][2], CIRCLE_OP);
    textFont(font);
    textSize(14);
    text(
